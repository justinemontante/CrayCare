...
Trains ML classifiers for insight type and recommendation type.
Each input is the 5 sensor statuses + overall_status.
Each output is a category ID → actual text.
"""
...
def get_insight_label_id(overall_status, sensor_statuses):
    ...
    if ph_s != "OPTIMAL" and do_s != "OPTIMAL":
        return 6  # COMBO_PH_DO_ALK (ph_high)
...
def get_rec_label_id(overall_status, sensor_statuses):
    ...
    if bitmask & 0x155:  # any CRITICAL bit set
        return 7  # ACTION_NEEDED_GENERAL
...
print("[1] Generating training data from all 3^5 = 243 sensor status combinations...")

data = []
for combo in itertools.product([0, 1, 2], repeat=5):
    sensor_statuses = make_sensor_statuses(combo)

    # Determine which overall statuses are valid for this sensor combo
    if all(v == 0 for v in combo):
        overall_statuses = ["OPTIMAL", "WARNING", "CRITICAL"]
    else:
        non_opt_count = sum(1 for v in combo if v > 0)
        overall_statuses = []
        if non_opt_count == 1:
    ...
    for overall_status in overall_statuses:
        ...
        insight_label = get_insight_label_id(overall_status, sensor_statuses)
        rec_label = get_rec_label_id(overall_status, sensor_statuses)
        row = dict(zip(FEATURE_COLS, features))
        row["insight_label"] = insight_label
        row["rec_label"] = rec_label
        data.append(row)

df = pd.DataFrame(data)
print(f"   {len(df)} samples. Insight labels: {sorted(df['insight_label'].unique())}")
...
for lbl, cnt in zip(*np.unique(y_insight, return_counts=True)):
    print(f"   {lbl} ({INSIGHT_LABEL_NAMES[lbl]}): {cnt}")

# ── Train Recommendation Classifier ────────────────────────────
print("\n[3] Training Recommendation Classifier...")
...
for lbl, cnt in zip(*np.unique(y_rec, return_counts=True)):
    ...
    print(f"   {lbl} ({names.get(lbl, lbl)}): {cnt}")
