...
class HistoryWindowTests(unittest.TestCase):
    ...
    def test_future_and_invalid_timestamps_are_excluded(self):
        _, status, source = anomaly_window(
            self.frame([self.NOW + 600, float("nan")]), self.NOW
        )
        self.assertEqual(status, "insufficient")
    ...
    def test_hourly_job_does_not_run_model_on_stale_history(self):
        ...
        with (
            patch.object(main, "_get_db", return_value=db),
        ...
        # _analyze_tank persists via a batched write: current alias +
        # timestamped history doc, both carrying the same result payload.
