...
class LettuceHarvestFormPanel extends StatefulWidget {
  final bool isOwner;
  final VoidCallback? onSaved;
  const LettuceHarvestFormPanel({super.key, this.isOwner = true, this.onSaved});

  @override
  State<LettuceHarvestFormPanel> createState() => _LettuceHarvestFormPanelState();
}

class _LettuceHarvestFormPanelState extends State<LettuceHarvestFormPanel> {
    ...
    final picked = await showDatePicker(
      ...
      builder: (ctx, child) => Theme(
        data: Theme.of(ctx).copyWith(
          colorScheme: Theme.of(ctx).colorScheme.copyWith(primary: AppColors.success),
        ),
        child: child!,
      ),
    );
  ...
  void _handleSave() {
    ...
    if (count > service.currentQuantity) {
      showBeautifulSnackbar(context, 'Harvest count exceeds available plants (${service.currentQuantity}).', false);
      return;
    }
    service.addLettuceHarvestRecord(harvestedCount: count, totalWeightKg: weight);
    widget.onSaved?.call();
  ...
  Widget build(BuildContext context) {
    final avgGrams = (double.tryParse(_weightCtrl.text) ?? 0) > 0 && (int.tryParse(_countCtrl.text) ?? 0) > 0
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.archive_rounded, size: 16, color: AppColors.success),
              ),
          ...
          _buildInputField('Plants Harvested', 'Number of plants', _countCtrl),
          _buildHarvestWarning(),
          const SizedBox(height: 12),
          _buildInputField('Total Harvest Weight (kg)', 'e.g. 1.5', _weightCtrl),
          if (avgGrams > 0) ...[
            ...
            Container(
              ...
              child: Row(
                children: [
                  Icon(Icons.monitor_weight_outlined, size: 14, color: AppColors.success),
                  const SizedBox(width: 8),
                  Text(
                    'Avg Weight: ${avgGrams.toStringAsFixed(1)} g/head',
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              style: ElevatedButton.styleFrom(
                backgroundColor: widget.isOwner ? AppColors.success : Colors.grey.shade300,
  ...
  Widget _buildHarvestWarning() {
    ...
    final available = LettuceService.instance.currentQuantity;
    if (count <= 0 || count <= available) return const SizedBox.shrink();
    return Padding(
        ...
        child: Column(
          children: [
            ...
            Padding(
              ...
              child: Row(
                children: [
                  const Icon(Icons.archive_outlined, size: 18, color: AppColors.success),
                  const SizedBox(width: 8),
  ...
  Widget _buildRecordCard(LettuceHarvestRecord r) {
        ...
        child: Row(
          children: [
            Container(
              ...
              child: const Icon(Icons.archive_rounded, size: 18, color: AppColors.success),
            ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text('${r.harvestedCount} plants | ${r.totalWeightKg.toStringAsFixed(2)}kg | Avg: ${r.avgWeightGrams.toStringAsFixed(1)}g',
