...
class _AdminScreenState extends State<AdminScreen> {
  ...
  StreamSubscription<QuerySnapshot<Map<String, dynamic>>>? _tanksSub;
  Map<String, bool> _initializedTanks = {};
  ...
  void _listenRealtime() {
          ...
          (snap) {
            final users =
                snap.docs.map((d) {
                  ...
                  (a, b) => (a['email'] as String? ?? '').compareTo(
                    b['email'] as String? ?? '',
                  ),
                );
            if (!mounted) return;
    ...
    _tanksSub?.cancel();
    _tanksSub = FirebaseFirestore.instance
        .collection('tanks')
        .snapshots()
        .listen(
          (snap) {
            ...
            if (!mounted) return;
            setState(() => _initializedTanks = initialized);
          },
          onError: (e) {
            debugPrint('[AdminScreen] tanks stream error: $e');
          },
        );
  }
                        ...
                        child: isLinked
                                      ...
                                      Expanded(
                                        child: Column(
                                          ...
                                          children: [
                                            ...
                                            const SizedBox(height: 2),
                                            Text(
                                              'Data routes to ${_displayName(user)}\'s account',
                                              style: const TextStyle(
                                                fontSize: 11,
                                                color: AppColors.subtitleText,
                                              ),
                            ...
                            ? Row(
                                children: [
                                  ...
                                  const Expanded(
                                    child: Text(
                                      'Owner must complete Tank Setup before hardware can be linked',
                                      style: TextStyle(
                                        fontSize: 12,
                                        color: AppColors.mutedText,
                                        fontWeight: FontWeight.w500,
          ...
          children: [
            ...
            OutlinedButton.icon(
              ...
              style: OutlinedButton.styleFrom(
                ...
                side: BorderSide(color: AppColors.primaryWith(0.35)),
              ),
            ),
          ],
        ),
  ...
  Widget _buildUserSearchField() {
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        filled: true,
        fillColor: AppColors.darkWith(0.025),
        ...
        focusedBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(13),
          borderSide: BorderSide(color: AppColors.primaryWith(0.45)),
        ),
      ),
    );
  }
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          const Text(
            'Try another name, email, or account filter.',
            style: TextStyle(fontSize: 10.5, color: AppColors.subtitleText),
          ),
        ],
      ),
    );
  ...
  Widget _buildStatsBar() {
    ...
    final ownerAccounts = _users
        .where((u) => _normalizedRole(u) == 'owner')
        .length;
    final adminAccounts = _users
        .where((u) => _normalizedRole(u) == 'admin')
        .length;
    final disabledAccounts = _users
        .where((u) => _normalizedStatus(u) == 'disabled')
        .length;
