...
/// `legacyAverage` keeps older Firestore records readable during migration.
double deriveSamplingAverage({
  required num? total,
  required num? sampleSize,
  double? legacyAverage,
}) {
  final count = sampleSize?.toDouble() ?? 0;
  if (total != null && count > 0) return total.toDouble() / count;
  return legacyAverage ?? 0.0;
}
...
class TankService extends ChangeNotifier {
  ...
  int get daysInCulture {
    ...
    return today
        .difference(
          DateTime(_stockingDate.year, _stockingDate.month, _stockingDate.day),
        )
        .inDays
        .clamp(0, 1000000)
        .toInt();
  ...
  bool get hasSamplingThisWeek {
    ...
    for (final entry in _samplingHistory) {
      ...
      if (!entryDate.isBefore(monday) && !entryDate.isAfter(sunday)) {
        return true;
      }
    }
    return false;
  ...
  List<CrayfishBatch> get harvestHistory =>
      List.unmodifiable(_harvestHistory.reversed);
  ...
  CrayfishBatch? get selectedBatch => _selectedBatchId != null
  ...
  Future<String> _resolveTankIdForUser(String uid) async {
    try {
      ...
      // A fresh Firebase authentication session is visible to every service
      // before AuthService finishes creating the profile. Wait for that
      // canonical provisioning path instead of creating a competing partial
      // profile, which previously caused first-time Google sign-in to fail.
      for (var attempt = 0; !profile.exists && attempt < 10; attempt++) {
      ...
      if (!profile.exists || data == null || role != 'owner') return '';
      return uid;
    } catch (e) {
  ...
  void _parseBatchesFromSnapshot(QuerySnapshot<Map<String, dynamic>> snap) {
    ...
    final currentSelectionStillExists =
        _selectedBatchId != null &&
        list.any((b) => b.batchId == _selectedBatchId);
  ...
  Future<void> _hydrateFinalSamplingAverages(
    List<CrayfishBatch> batches,
  ) async {
    final completed = batches
        .where(
          (batch) =>
              batch.status == 'harvested' || batch.status == 'superseded',
        )
        .toList();
    ...
    await Future.wait(
      completed.map((batch) async {
        try {
          final snapshot = await _samplingRef(batch.batchId).get();
          Map<String, dynamic>? record;
          DateTime? latestDate;
          for (final doc in snapshot.docs) {
            final data = doc.data();
            final date = _readTankDate(data['sampling_date']);
            if (date != null &&
          ...
          debugPrint(
            '[TankService] Failed to derive final sampling values for '
            '${batch.batchId}: $e',
          );
        }
      }),
    ...
    _batches = _batches.map((batch) {
      ...
      if (values == null) return batch;
      return batch.withFinalSamplingAverages(abw: values[0], abl: values[1]);
    }).toList();
    _harvestHistory = _batches
        .where(
          (batch) =>
              batch.status == 'harvested' || batch.status == 'superseded',
        )
            ...
            _mortalityHistory =
                snap.docs
                    .map((doc) {
                      ...
                      if (date == null || countRaw is! num) return null;
  ...
  Future<void> initializeGrowOut(
    ...
    if (requestedName.contains('/') || requestedName.length > 100) {
      throw ArgumentError(
        'Batch name cannot contain "/" and must be 100 characters or fewer',
      );
    }

    // This is the sole owner-facing point that provisions tanks/{uid} and
    // its default sensors, actuators, and feeder status. Registration itself
    // intentionally leaves the owner without a tank document.
    await DatabaseService.instance.ensureOwnerTankProvisioned(
      _tankOwnerUid,
    ...
    if (editExisting) {
      ...
      if (requestedName.isNotEmpty && requestedName != currentId) {
        throw ArgumentError(
          'The batch name cannot be changed after initialization',
        );
      }
      if (_samplingHistory.any((entry) => !entry.isBaseline) ||
        ...
        throw StateError(
          'Initialization cannot be edited after operational records exist',
        );
      }

      final initialWeight = totalWeight / sampleCount;
      final initialLength = totalLength / sampleCount;
      final writes = FirebaseFirestore.instance.batch();
      writes.set(_batchesRef.doc(currentId), {
        ...
        'final_abl': FieldValue.delete(),
      }, SetOptions(merge: true));
      writes.set(_samplingRef(currentId).doc('baseline'), {
        'sampling_date': _writeTankTimestamp(date),
    ...
    try {
      ...
      if (existingActive != null) {
        writes.set(_batchesRef.doc(existingActive.batchId), {
          ...
          'ended_at': _writeTankTimestamp(DateTime.now()),
          'days_in_culture': FieldValue.delete(),
          'initial_abw': FieldValue.delete(),
          'initial_abl': FieldValue.delete(),
          'final_abw': FieldValue.delete(),
          'final_abl': FieldValue.delete(),
      ...
      writes.set(_batchesRef.doc(bid), {
        'batch_id': bid,
        'batch_status': 'active',
        'stocking_date': _writeTankTimestamp(_stockingDate),
        'harvest_date': null,
    ...
    _addActivity(
      'Initialized new grow-out batch with $initial population',
      'init',
      customDate: date,
    );
  ...
  Future<void> addMortality(int val, {DateTime? date}) async {
    ...
    if (val > inTankCount) {
      throw ArgumentError(
        'Mortality count exceeds in-tank population ($inTankCount)',
      );
    }
    ...
    try {
      await _fs.runTransaction((transaction) async {
        ...
        final initial =
            (data['initial_count'] as num?)?.toInt() ?? _initialCount;
        final batchMortality = (data['total_mortality'] as num?)?.toInt() ?? 0;
        // Legacy active batches did not always update this aggregate. Prefer
        // the subscribed record total whenever it is higher.
        final existingMortality = batchMortality > _mortality
            ? batchMortality
            : _mortality;
        final existingHarvest =
            (data['harvest_count'] as num?)?.toInt() ?? _totalHarvested;
        final available = (initial - existingMortality - existingHarvest).clamp(
          0,
          initial,
        );
        if (val > available) {
          throw StateError(
            'Mortality count exceeds current in-tank population ($available)',
          );
        }
        committedMortality = existingMortality + val;
        transaction.set(recordRef, {
          'mortality_date': _writeTankTimestamp(mEntry.date),
          'mortality_count': mEntry.count,
          'created_at': FieldValue.serverTimestamp(),
        });
        transaction.set(batchRef, {
          'total_mortality': committedMortality,
          'current_count': (initial - committedMortality - existingHarvest)
              .clamp(0, initial),
        }, SetOptions(merge: true));
      });
    ...
    try {
      await _fs.runTransaction((transaction) async {
        ...
        if (!recordSnap.exists ||
            recordSnap.data() == null ||
            !batchSnap.exists ||
            batchSnap.data() == null) {
          throw StateError('Mortality record or batch no longer exists');
        }
        final actualOld =
            (recordSnap.data()!['mortality_count'] as num?)?.toInt() ??
            last.count;
        final data = batchSnap.data()!;
        final initial =
            (data['initial_count'] as num?)?.toInt() ?? _initialCount;
        ...
        if (committedMortality < 0 || committedMortality > maxMortality) {
          throw StateError(
            'Updated mortality exceeds available population ($maxMortality)',
          );
        }
      ...
      _addActivity(
        'Updated last mortality record to $newCount (Total: $_mortality)',
        'mortality',
      );
    } catch (e) {
  ...
  Future<void> addHarvestRecord({
    ...
    if (harvestedCount <= 0) {
      throw ArgumentError('Harvested count must be greater than 0');
    }
    if (totalWeightKg <= 0) {
      throw ArgumentError('Total harvest weight must be greater than zero');
    }
    ...
    if (harvestedCount > inTankCount) {
      throw ArgumentError(
        'Harvest count exceeds in-tank population ($inTankCount)',
      );
    }
    ...
    try {
      await _fs.runTransaction((transaction) async {
        ...
        final initial =
            (existing['initial_count'] as num?)?.toInt() ?? _initialCount;
        final batchDeaths = (existing['total_mortality'] as num?)?.toInt() ?? 0;
        final deaths = batchDeaths > _mortality ? batchDeaths : _mortality;
        final existingHarvest =
            (existing['harvest_count'] as num?)?.toInt() ?? 0;
        final available = (initial - deaths - existingHarvest).clamp(
          0,
          initial,
        );
        if (harvestedCount > available) {
          throw StateError(
            'Harvest count exceeds current available population ($available)',
          );
        }
        ...
        transaction.set(recordRef, {
          ...
          'harvest_date': _writeTankTimestamp(now),
        ...
        transaction.set(batchRef, {
          ...
          'current_count': (initial - deaths - committedTotal).clamp(
            0,
            initial,
          ),
  ...
  Future<void> updateLastHarvestRecord({
    ...
    final resolvedBatchId = last.batchId.isNotEmpty
        ? last.batchId
        : (_selectedBatchId ?? '');
    if (resolvedBatchId.isEmpty) throw ArgumentError('No batch selected');
    ...
    try {
      await _fs.runTransaction((transaction) async {
        ...
        final actualOldCount =
            (recordData['harvest_count'] as num?)?.toInt() ??
            last.harvestedCount;
        final actualOldWeight =
            (recordData['total_weight_kg'] as num?)?.toDouble() ??
            last.totalWeightKg;
        final initial =
            (batchData['initial_count'] as num?)?.toInt() ?? _initialCount;
        final batchDeaths =
        ...
        if (committedTotal < 0 || committedTotal > maxSurvivors) {
          throw StateError(
            'Updated harvest exceeds available population ($maxSurvivors)',
          );
        }
        final previousWeight =
            (batchData['harvest_weight_grams'] as num?)?.toDouble() ?? 0.0;
        final newWeight =
            previousWeight - (actualOldWeight * 1000) + (totalWeightKg * 1000);

        transaction.update(recordRef, {
          'batch_id': resolvedBatchId,
          'harvest_count': harvestedCount,
          'total_weight_kg': totalWeightKg,
          'abw_grams': FieldValue.delete(),
          'created_at': FieldValue.serverTimestamp(),
        });
        transaction.set(batchRef, {
          'harvest_count': committedTotal,
  ...
  Future<void> completeBatch({
    ...
    double? harvestWeightGrams,
    String? batchId,
  }) async {
    if (!_isInitialized || _isArchiveView) return;
    final now = DateTime.now();
    final activeBatchId = _batches
        .where((b) => b.status == 'active')
        .firstOrNull
        ?.batchId;
    final resolvedId =
        batchId ?? activeBatchId ?? 'Batch ${_batches.length + 1}';
    ...
    try {
      ...
      _addActivity(
        'Completed grow-out batch ($resolvedId). Harvested $harvestCount crayfish${harvestWeightGrams != null ? ', ${harvestWeightGrams.toStringAsFixed(1)}g total' : ''}.',
        'harvest',
      );
      _resetAll();
    } catch (e) {
