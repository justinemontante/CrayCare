...
Reviewed the current Flutter/Dart application, Firestore integration and rules, Node notification/routing functions, Python Water Quality Assessment pipeline, ESP32 production firmware, test tools, schemas, indexes, and deployment configuration.
...
6. **Test tools targeted removed collections.** Mock ingestion, Water Quality Assessment seeding/checking, and threshold configuration now use current staging/tank paths and current snake_case fields.
