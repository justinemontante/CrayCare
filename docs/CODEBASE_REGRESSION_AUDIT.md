...
Reviewed the current Flutter/Dart application, Firestore integration and rules, Node notification/routing functions, Python WQC pipeline, ESP32 production firmware, test tools, schemas, indexes, and deployment configuration.
...
- The pinned XGBoost model loads and produces a prediction.
...
6. **Test tools targeted removed collections.** Mock ingestion, WQC seeding/checking, and threshold configuration now use current staging/tank paths and current snake_case fields.
...
2. **Synthetic ML training:** WQC is prototype-validated on formula-labeled synthetic data, not field-validated biological ground truth.
