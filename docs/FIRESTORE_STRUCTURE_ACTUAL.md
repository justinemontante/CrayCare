...
ESP32 writes: `status` (`idle`|`dispensing`), `feedCount`, `hopperLevel`, `feederError`, `lastSeen`, `last_dispensed_at`, `last_dispensed_grams`
...
`command_type` (`feed_now`), `grams`, `issued_by`, `issued_at`
